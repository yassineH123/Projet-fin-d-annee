...
function getTransport() {
  // Compte Gmail simple (GMAIL_USER/GMAIL_PASS) — config historique du projet.
  ...
  // SMTP générique (SMTP_HOST/SMTP_PORT/...) — config alternative.
...
function fromAddress() {
  return process.env.GMAIL_USER || process.env.SMTP_FROM || process.env.SMTP_USER;
}
...
async function send({ to, subject, html }) {
  ...
  if (!transport) {
    console.log(`\n[EMAIL] To: ${to} | Subject: ${subject}\n[EMAIL] (SMTP/Gmail non configuré — email non envoyé)\n`);
    return;
  }
  await transport.sendMail({
    from: `"AtlasWay" <${fromAddress()}>`,
    to, subject, html,
  });
}
