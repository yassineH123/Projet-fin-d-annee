...
async function getMine(req, res, next) {
  try {
    const alerts = await RideAlert.find({ userId: req.user.id }).sort({ createdAt: -1 });
...
async function remove(req, res, next) {
  try {
    const alert = await RideAlert.findOne({ _id: req.params.id, userId: req.user.id });
...
async function triggerAlerts(ride) {
  try {
    ...
    if (ride.transportMode) where.transportMode = { $in: [ride.transportMode, null] };
    const alerts = await RideAlert.find(where);
