...
function StepDots({ step, isDriver }) {
...
function AnimatedStep({ children, direction = 'right' }) {
  return (
    <div
      style={{
        animation: `slideIn${direction === 'right' ? 'Right' : 'Left'} 0.3s ease-out`,
      }}
    >
      {children}
    </div>
...
export default function Onboarding() {
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: 'var(--bg-900)' }}>

      {/* Header */}
      <div className="flex items-center justify-center py-6" style={{ borderBottom: '1px solid var(--border-color)' }}>
        <div className="flex items-center gap-2">
          <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: 'rgba(193,39,45,0.15)' }}>
            <Car className="text-primary-400" size={22} />
          </div>
          <span className="font-black text-2xl">
            <span className="text-white">Atlas</span><span style={{ color: '#C1272D' }}>Way</span>
          </span>
        </div>
      </div>
        ...
        {step === STEPS.CHOICE && (
          <AnimatedStep direction={direction}>
            <div className="w-full max-w-3xl">
              ...
              <StepDots step={step} isDriver={choice === 'driver'} />

              <div className="text-center mb-10">
                <div className="inline-flex items-center gap-2 rounded-full px-4 py-2 mb-5 text-sm font-semibold"
                  style={{ background: 'rgba(193,39,45,0.1)', border: '1px solid rgba(193,39,45,0.2)', color: '#C1272D' }}>
                  <CheckCircle size={15} /> Compte créé avec succès
                </div>
              ...
              <div className="flex justify-center gap-10 mb-10">
                {[['12 000+', 'Voyageurs'], ['45+', 'Villes'], ['4.8/5', 'Note moyenne']].map(([val, label]) => (
                  <div key={label} className="text-center">
                    <p className="text-2xl font-black" style={{ color: '#C1272D' }}>{val}</p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-8">
                ...
                <button
                  ...
                  className="group p-7 rounded-2xl border-2 text-left transition-all duration-200"
                  style={{
                    background: choice === 'passenger' ? 'rgba(193,39,45,0.08)' : 'var(--card-bg)',
                    borderColor: choice === 'passenger' ? '#C1272D' : 'var(--border-color)',
                    transform: choice === 'passenger' ? 'translateY(-2px)' : 'none',
                    boxShadow: choice === 'passenger' ? '0 8px 32px rgba(193,39,45,0.2)' : 'none',
                  }}
                >
                  <div className="w-14 h-14 rounded-2xl flex items-center justify-center mb-5 transition-colors"
                    style={{ background: choice === 'passenger' ? '#C1272D' : 'var(--bg-700)' }}>
                  ...
                  <ul className="space-y-2">
                    {['Rechercher des trajets', 'Réserver en un clic', 'Communiquer avec le conducteur'].map(f => (
                      <li key={f} className="flex items-center gap-2 text-slate-400 text-xs">
                        <CheckCircle size={13} style={{ color: '#C1272D' }} className="shrink-0" /> {f}
                      </li>
                    ))}
                  </ul>
                  {choice === 'passenger' && (
                    <div className="mt-4 flex items-center gap-1.5 text-sm font-semibold" style={{ color: '#C1272D' }}>
                ...
                <button
                  ...
                  className="group p-7 rounded-2xl border-2 text-left transition-all duration-200"
                  style={{
                    background: choice === 'driver' ? 'rgba(22,163,74,0.08)' : 'var(--card-bg)',
                    borderColor: choice === 'driver' ? '#16a34a' : 'var(--border-color)',
                    transform: choice === 'driver' ? 'translateY(-2px)' : 'none',
                    boxShadow: choice === 'driver' ? '0 8px 32px rgba(22,163,74,0.2)' : 'none',
                  }}
                >
                  <div className="w-14 h-14 rounded-2xl flex items-center justify-center mb-5 transition-colors"
                    style={{ background: choice === 'driver' ? '#16a34a' : 'var(--bg-700)' }}>
                  ...
                  {choice === 'driver' && (
                    <div className="mt-4 flex items-center gap-1.5 text-sm font-semibold text-green-400">
                      <CheckCircle size={16} /> Sélectionné
                    </div>
                  )}
                </button>
            ...
            <div className="w-full max-w-md">
              ...
              <div className="card flex flex-col gap-5">
                ...
                <div className="flex flex-col items-center gap-3">
                  <div onClick={() => photoRef.current.click()} className="relative w-24 h-24 rounded-full cursor-pointer group">
                    {photoPreview
                      ? <img src={photoPreview} alt="" className="w-24 h-24 rounded-full object-cover ring-2 ring-green-500" />
                      : <div className="w-24 h-24 rounded-full flex flex-col items-center justify-center group-hover:border-green-500 transition"
                          style={{ background: 'var(--bg-700)', border: '2px dashed var(--border-muted)' }}>
                    ...
                    <div className="absolute -bottom-1 -right-1 w-8 h-8 bg-green-500 rounded-full flex items-center justify-center border-2"
                      style={{ borderColor: 'var(--bg-900)' }}>
                ...
                <div className="flex gap-3 pt-2">
                  <button onClick={() => goTo(STEPS.CHOICE, 'left')} className="btn-secondary flex-1 flex items-center justify-center gap-2">
          ...
          <AnimatedStep direction={direction}>
            <div className="w-full max-w-md">
              ...
              <div className="card flex flex-col gap-5">
                {/* Photo voiture */}
                <div>
                  <label className="text-sm text-slate-400 mb-2 block">Photo du véhicule <span className="text-red-400">*</span></label>
                  <div
                    ...
                    style={{
                      borderColor: carPreview ? '#16a34a' : 'var(--border-muted)',
                ...
                {/* Plaque */}
                <div>
                  <label className="text-sm text-slate-400 mb-1.5 block">Numéro de plaque <span className="text-red-400">*</span></label>
                ...
                <div className="flex gap-3 pt-2">
                  <button onClick={() => goTo(STEPS.DRIVER_INFO, 'left')} className="btn-secondary flex-1 flex items-center justify-center gap-2">
                    <ArrowLeft size={16} /> Retour
                  </button>
                  <button
