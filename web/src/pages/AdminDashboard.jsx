...
import { Users, Car, BookOpen, Star, Search, Shield, Ban, Trash2, CheckCircle } from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  const blockUser = async (id) => {
    ...
    toast.success('Utilisateur bloqué'); fetchUsers();
  };
  const unblockUser = async (id) => {
    await api.patch(`/admin/users/${id}/unblock`);
    toast.success('Utilisateur débloqué'); fetchUsers();
  };
  const deleteUser = async (id) => {
    if (!window.confirm('Supprimer cet utilisateur ?')) return;
    await api.delete(`/admin/users/${id}`);
    toast.success('Supprimé'); fetchUsers();
  };
  const cancelRide = async (id) => {
    await api.patch(`/admin/rides/${id}/cancel`);
    toast.success('Trajet annulé'); fetchRides();
  };
