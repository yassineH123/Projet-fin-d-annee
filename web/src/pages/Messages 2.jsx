...
export default function Messages() {
  ...
  useEffect(() => {
    ...
    socket.on('online_users', (ids) => setOnlineSet(new Set(ids)));
    socket.on('presence_update', ({ userId: uid, online }) => {
      setOnlineSet(prev => {
          ...
          {convs.length === 0 && !pending ? (
            ...
            return (
              <button key={conv.id} onClick={() => { setActive(conv); setPending(null); }}
                ...
                <div className="flex items-center gap-3">
                  {info.isGroup ? <GroupIcon members={info.members} /> : (
                    <div className="relative shrink-0">
                      <Avatar user={{ firstName: info.name, photo: info.photo }} />
                      {info.other && onlineSet.has(info.other.id) && (
                        <span className="absolute bottom-0 right-0 w-3 h-3 rounded-full border-2"
                          style={{ background: '#22c55e', borderColor: 'var(--card-bg)' }} title="En ligne" />
                      )}
                    </div>
                  )}
            ...
            <>
              ...
              <div className="border-b pb-3 mb-3 flex items-center gap-3" style={{ borderColor: 'var(--border-color)' }}>
                ...
                <div>
                  ...
                  {activeInfo?.other && (
                    onlineSet.has(activeInfo.other.id)
                      ? <p className="text-xs flex items-center gap-1" style={{ color: '#22c55e' }}>
                          <span className="w-2 h-2 rounded-full inline-block" style={{ background: '#22c55e' }} /> En ligne
                        </p>
